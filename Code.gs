...
function getSelectedText() {
  var selection = SpreadsheetApp.getActiveDocument().getSelection();
  if (selection) {
    var text = [];
...
function insertText(newText) {
  ...
  if (selection) {
    ...
    if (elements.length == 1 &&
        ...
        SpreadsheetApp.ElementType.INLINE_IMAGE) {
    ...
    var cursor = SpreadsheetApp.getActiveDocument().getCursor();
